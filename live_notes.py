...
from state import (
    ITEM_SECTIONS, SECTION_PREFIXES, DUP_THRESHOLD, MAX_UPDATE_WORDS, STOPWORDS,
    DEFAULT_STATE, ID_PREFIX_RE,
    tokens, containment, flatten_item, migrate_state, load_state, save_state,
    mint_id, live_items, find_item, total_items, find_duplicate, norm_id,
    clean_text, validate_ops, add_item, apply_ops, apply_edits,
)
...
# Schema, id minting, load/save, dedup and the op-application functions
# (apply_ops, apply_edits) live in state.py — this module only renders and
# orchestrates.
...
# --- Ops: the only way the model changes notes (state.py: norm_id,
# clean_text, validate_ops, add_item, apply_ops, apply_edits) ---
    ...
    for attempt in range(MAX_MERGE_RETRIES + 1):
        try:
            ...
            if valid:
                ...
                print(f"  [merge] {applied} op(s). \033[31mgist: {candidate.get('gist', '(none)')}\033[0m")
                return state
